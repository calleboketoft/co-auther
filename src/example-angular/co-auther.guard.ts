...
export class CoAutherGuard implements CanActivate {
  constructor(private coAuther: CoAuther, private router: Router) {}
  canActivate(route, state) {
    ...
    let unauthedLoggedInAttempt =
      routeRequest === ROUTE_LOGGED_IN && routeResponse === ROUTE_AUTHENTICATE
    let authedLoggedInAttempt =
      routeRequest === ROUTE_LOGGED_IN &&
      routeResponse === ROUTE_INITIAL_REQUEST
